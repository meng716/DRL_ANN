...
variant = 1  # problem variant (0 for base variant, 1 for first extension, 2 for second extension)
...
def evaluate_policy(visualization_manager):
    visualization_manager.reset_mode(mode)
    reward = 0.
    for i in range(100):
        env.reset(mode)
        visualization_manager.check_is_record(i)
        visualization_manager.record_data_for_ani(env.plot_buffer, reward=0, time=0)
        reward_old = reward
        for j in range(max_episode_steps):
            act = greedy_policy()
            rew, _, _ = env.step(act)
            reward += rew
            obs_list.append([env.agent_loc[0],env.agent_loc[1],env.agent_load, rew,act])
            visualization_manager.record_data_for_ani(env.plot_buffer, rew, j + 1)
        reward_list.append(reward-reward_old)
        visualization_manager.increment_episode_count()
    avg_reward = reward / 100
    print(avg_reward)
    pd.DataFrame(obs_list).to_csv(f"./logs/movement/Variant{variant}/obv_greedy")
    pd.DataFrame(reward_list).to_csv(f"./logs/movement/Variant{variant}/reward_greedy")
### run greedy policy evaluation

if __name__ == '__main__':
    print(os.getcwd())

    no_training_episodes = 2500
    no_validation_episodes = 100
    visualization_mode = {"training": False, "validation": True}
    ...
    visualization_manager.current_time = current_time

    evaluate_policy(visualization_manager)

    visualization_manager.prepare_data_for_ani()
    visualization_manager.create_env_animation(save_path=f"./outputs/greedy/variant{variant}", save_pngs=False, fps=5)
